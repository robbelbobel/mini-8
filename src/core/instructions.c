...
/** CLS RET **/
void __cls(chip_t* chip, const __uint16_t instr){
    for(__uint16_t i = 0; i < DISPLAY_BUFFER_SIZE; i++) chip -> display[i] = 0;
}

void __ret(chip_t* chip, const __uint16_t instr){
...
/** SE **/
void __se_v_byte(chip_t* chip, const __uint16_t instr){
    if(chip -> v[(instr & 0x0F00) >> 8] == (instr & 0xFF)) chip -> pc += 2;
}

void __se_v_v(chip_t* chip, const __uint16_t instr){
    if(chip -> v[(instr & 0x0F00) >> 8] == chip -> v[(instr & 0x00F0) >> 8]) chip -> pc += 2;
}

void __sne_v_byte(chip_t* chip, const __uint16_t instr){
    if(chip -> v[(instr & 0x0F00) >> 8] != (instr & 0xFF)) chip -> pc += 2;
}

void __sne_v_v(chip_t* chip, const __uint16_t instr){
    if(chip -> v[(instr & 0x0F00) >> 8] != chip -> v[(instr & 0x00F0) >> 8]) chip -> pc += 2;
}

/** LD **/
void __ld_v_byte(chip_t* chip, const __uint16_t instr){
    chip -> v[(instr & 0x0F00) >> 8] = (instr & 0xFF);
}

void __ld_v_v(chip_t* chip, const __uint16_t instr){
    chip -> v[(instr & 0x0F00) >> 8] = chip -> v[(instr & 0x00F0) >> 4];
}

void __ld_i_addr(chip_t* chip, const __uint16_t instr){
    chip -> i = instr & 0xFFF;
}

void __ld_v_dt(chip_t* chip, const __uint16_t instr){
    chip -> v[(instr & 0x0F00) >> 8] = chip -> dt;
}

void __ld_v_k(chip_t* chip, const __uint16_t instr){
    ;
}

void __ld_dt_v(chip_t* chip, const __uint16_t instr){
    chip -> dt = chip -> v[(instr & 0x0F00) >> 8];
}

void __ld_st_v(chip_t* chip, const __uint16_t instr){
    chip -> st = chip -> v[(instr & 0x0F00) >> 8];
}

void __ld_f_v(chip_t* chip, const __uint16_t instr){
...
void __ld_i_v(chip_t* chip, const __uint16_t instr){
    for(__uint8_t n = 0; n < 16; n++) chip -> memory[chip -> i + n] = chip -> v[n];
}

void __ld_v_i(chip_t* chip, const __uint16_t instr){
    for(__uint8_t n = 0; n < 16; n++) chip -> v[n] = chip -> memory[chip -> i + n];
}

/** ADD **/
void __add_v_byte(chip_t* chip, const __uint16_t instr){
    chip -> v[(instr & 0x0F00) >> 8] += instr & 0xFF;
}

void __add_v_v(chip_t* chip, const __uint16_t instr){
    chip -> v[(instr & 0x0F00) >> 8] = chip -> v[(instr & 0x00F0) >> 4];
}

void __add_i_v(chip_t* chip, const __uint16_t instr){
    chip -> i += chip -> v[(instr & 0x0F00) >> 8];
}

/** BIT OPERATIONS **/
void __or(chip_t* chip, const __uint16_t instr){
    chip -> v[(instr & 0x0F00) >> 8] |= chip -> v[(instr & 0x00F0) >> 8];
}

void __and(chip_t* chip, const __uint16_t instr){
    chip -> v[(instr & 0x0F00) >> 8] &= chip -> v[(instr & 0x00F0) >> 8];
}

void __xor(chip_t* chip, const __uint16_t instr){
    chip -> v[(instr & 0x0F00) >> 8] ^= chip -> v[(instr & 0x00F0) >> 8];
}

/** SUB **/
void __sub(chip_t* chip, const __uint16_t instr){
    chip -> v[(instr & 0x0F00) >> 8] -= chip -> v[(instr & 0x00F0) >> 4];
}

void __subn(chip_t* chip, const __uint16_t instr){
    chip -> v[(instr & 0x0F00) >> 8] = chip -> v[(instr & 0x00F0) >> 4] - chip -> v[(instr & 0x0F00) >> 8];
}

/** SHIFT **/
void __shr(chip_t* chip, const __uint16_t instr){
    chip -> v[(instr & 0x0F00) >> 8] >>= 1;
}

void __shl(chip_t* chip, const __uint16_t instr){
    chip -> v[(instr & 0x0F00) >> 8] <<= 1;
}
...
void __drw(chip_t* chip, const __uint16_t instr){
    ...
    for(uint8_t i = 0; i < height; i++){
        ...
        for(uint8_t j = 0; j < 8; j++){
            chip -> display[(y + i) * (DISPLAY_WIDTH / 8) + ((x + j) / 8)] ^= row & (0b1 << (7 - j));
        }
    }

    for(uint8_t i = 0; i < DISPLAY_HEIGHT; i++){
        for(uint8_t j = 0; j < DISPLAY_WIDTH / 8; j++){
            printf("%x", chip -> display[i * (DISPLAY_WIDTH / 8) + j]);
        }
        printf("\n");
    }
